...
- 2026-09-13: Org 1 — `POST /api/orgs` creates a second principal (`type=org`, no `owner_address`) under the session; one org per account. Header toggles `/cabinet` ↔ `?house=`. Owner Settings tab can `PATCH`/`DELETE /api/orgs`. Personal `/cabinet` and `ensureHouseForOwner` unchanged. OpenAPI `0.43.0`.
...
- 2026-09-13: Org 5 — org Access tab invites a wallet with section grants (`house_members.grants`: agents, treasury, rules). Rules are not default; empty grants are the feed only. Leftover `type=org` with `owner_address` is not a company house. Guest keeps personal `/cabinet`; invited orgs show in the header. Yes/no on escalate is Contacts, not a cabinet grant. Personal houses unchanged. OpenAPI `0.47.0`.
- 2026-09-14: GenLayer `genlayer-js` pinned `2.0.0-rc.1`. Court poll uses tx `lifecycle` (Consensus v0.6 `LEADER_REVEALING` stays pending). RPC override `GENLAYER_RPC_URL` / `studio-next` alias. No Transaction Kit — IC writes stay on the house wallet.
- 2026-09-14: Org 6 — Connect stores `court_label` / `court_cap`; `buildJudgeInput` adds them as `court_agent` / objection label+cap when set (not the prompt). IC prompt notes that identity. Houses redeploy (`court_abi` 5). Personal houses unchanged if fields stay empty. OpenAPI `0.48.0`.
